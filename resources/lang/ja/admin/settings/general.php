...
'ldap_location_help'             => '[LDAPロケーション] フィールドは、<strong>ベース バインド DN で OU が使用されていない</strong>場合に使用する必要があります。OU 検索が使用されている場合は、これを空白のままにしてください。',
    ...
    'ldap_activated_flag_help'  => 'この値は、同期されたユーザーがInventory Managerにログインできるかどうかを決定するために使用されます。<strong>アイテムをチェックインまたはチェックアウトする機能には影響しません。</strong>また、<strong>値</strong>ではなく、AD/LDAP内の<strong>属性名</strong>である必要があります。<br><br>
...
このフィールドにAD/LDAP内に存在しないフィールド名が設定されている場合、またはAD/LDAPフィールドの値が<code>0</code>または<code>false</code>に設定されている場合、<strong>ユーザログインは無効化</strong>されます。AD/LDAPフィールドの値が<code>1</code>または<code>true</code>に設定されている場合、または<em>その他のテキストが設定されている場合</em>は、ユーザーは<strong>ログインすることが可能</strong>です。ADでこのフィールドが空白の場合、<code>userAccountControl</code>属性を尊重し、通常、サスペンドされていないユーザーのログインを許可しています。',
    ...
    'load_remote_help_text'		=> 'Inventory Managerのインストールは、外部からスクリプトを読み込むことが可能です。',
    'login'                     => 'ログイン試行',
...
これは認証プロバイダのセッションを正しく終了させる場合に便利です。',
    ...
    'show_images_in_email_help'   => 'Inventory Managerが閉鎖されたネットワーク内にある場合や、アクセスするためにVPNの利用が必要な場合はこのボックスのチェックを外してください。ネットワーク外のユーザは、配信されたメールにある画像を読み込むことはできません。',
    'site_name'                 => 'サイト名',
    'integrations'               => 'サービス連携',
    'slack'                     => 'Slack',
    ...
    'InventoryManager_version'  			=> 'Inventory Manager バージョン',
    'support_footer'            => 'フッターのリンクをサポートします。 ',
    'support_footer_help'       => 'Inventory Managerサポート情報とユーザーマニュアルへのリンクを確認する',
    'version_footer'            => 'フッターにバージョン情報を表示 ',
    'version_footer_help'       => 'Inventory Managerのバージョンとビルド番号を表示するか、管理者のみに表示させるかを設定できます。',
    'system'                    => 'システム情報',
    'update'                    => '設定を更新',
    'value'                     => '価値',
    'brand'                     => 'ブランディング',
    ...
    'width_w'        => 'w',
    'height_h'        => 'h',
    'show_url_in_emails'                => 'Inventory Managerへのリンク',
    'show_url_in_emails_help_text'      => '電子メールのフッターでInventory Managerのインストールにリンクしたくない場合は、このボックスのチェックを外してください。ほとんどのユーザーが決してログインしない場合に便利です。',
    'text_pt'        => 'pt',
    ...
    'barcodes_help' => 'キャッシュされたバーコードを削除しようとします。 これは通常、バーコード設定が変更された場合や、Inventory ManagerのURLが変更された場合にのみ使用されます。 バーコードは次にアクセスしたときに再生成されます。',
