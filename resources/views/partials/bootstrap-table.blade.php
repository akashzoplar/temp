...
<script nonce="{{ csrf_token() }}">
    $(function () {
        ...
        $('.InventoryManager-table').bootstrapTable('destroy').each(function () {
            ...
            pageSize: {{  (($InventoryManagerSettings->per_page!='') && ($InventoryManagerSettings->per_page > 0)) ? $InventoryManagerSettings->per_page : 20 }},
    ...
    function dateRowCheckStyle(value) {
        if ((value.days_to_next_audit) && (value.days_to_next_audit < {{ $InventoryManagerSettings->audit_warning_days ?: 0 }})) {
    ...
    $('.InventoryManager-table').on('check.bs.table .btSelectItem', function (row, $element) {
    ...
    $('.InventoryManager-table').on('check-all.bs.table', function (event, rowsAfter) {
    ...
    $('.InventoryManager-table').on('uncheck.bs.table .btSelectItem', function (row, $element) {
        var tableId =  $(this).data('id-table');
    ...
    $('.InventoryManager-table').on('uncheck.bs.table', function () {
    ...
    $('.InventoryManager-table').on('uncheck-all.bs.table', function (event, rowsAfter, rowsBefore) {
    ...
    function polymorphicItemFormatter(value) {
        ...
        if ((value) && (value.type)) {
            ...
            return '<nobr><a href="{{ config('app.url') }}/' + item_destination +'/' + value.id + '" data-tooltip="true" title="' + value.type + '"><i class="' + item_icon + ' text-{{ $InventoryManagerSettings->skin!='' ? $InventoryManagerSettings->skin : 'blue' }} "></i> ' + value.name + '</a></nobr>';
    ...
    function auditImageFormatter(value){
        if (value){
            return '<a href="' + value.url + '" data-toggle="lightbox" data-type="image"><img src="' + value.url + '" style="max-height: {{ $InventoryManagerSettings->thumbnail_max_h }}px; width: auto;" class="img-responsive"></a>'
        }
    }
   ...
   function imageFormatter(value, row) {
        ...
        if (value) {
            ...
            return '<a href="' + value + '" data-toggle="lightbox" data-type="image"><img src="' + value + '" style="max-height: {{ $InventoryManagerSettings->thumbnail_max_h }}px; width: auto;" class="img-responsive" alt="' + altName + '"></a>';
        }
    }
    ...
    function labelRadioFormatter(value, row, index, field) {
        if (row) {
            return row.name == '{{ str_replace("\\", "\\\\", $InventoryManagerSettings->label2_template) }}';
        }
    }
    ...
    function cleanFloat(number) {
        ...
        if ("{{$InventoryManagerSettings->digit_separator}}" == "1.234,56") {
    ...
    function numberWithCommas(value) {
        ...
        if ((value) && ("{{$InventoryManagerSettings->digit_separator}}" == "1.234,56")){
    ...
    $(function () {
        $('#bulkEdit').click(function () {
            var selectedIds = $('.InventoryManager-table').bootstrapTable('getSelections');
