...
<head>
    ...
    <link rel="apple-touch-icon"
          href="{{ ($InventoryManagerSettings) && ($InventoryManagerSettings->favicon!='') ?  Storage::disk('public')->url(e($InventoryManagerSettings->logo)) :  config('app.url').'/img/InventoryManager-logo-bug.png' }}">
    <link rel="apple-touch-startup-image"
          href="{{ ($InventoryManagerSettings) && ($InventoryManagerSettings->favicon!='') ?  Storage::disk('public')->url(e($InventoryManagerSettings->logo)) :  config('app.url').'/img/InventoryManager-logo-bug.png' }}">
    <link rel="shortcut icon" type="image/ico"
          href="{{ ($InventoryManagerSettings) && ($InventoryManagerSettings->favicon!='') ?  Storage::disk('public')->url(e($InventoryManagerSettings->favicon)) : config('app.url').'/favicon.ico' }} ">
    ...
    @if (($InventoryManagerSettings) && ($InventoryManagerSettings->allow_user_skin==1) && Auth::check() && Auth::user()->present()->skin != '')
        <link rel="stylesheet" href="{{ url(mix('css/dist/skins/skin-'.Auth::user()->present()->skin.'.min.css')) }}">
    @else
        <link rel="stylesheet"
              href="{{ url(mix('css/dist/skins/skin-'.($InventoryManagerSettings->skin!='' ? $InventoryManagerSettings->skin : 'blue').'.css')) }}">
    @endif
    {{-- page level css --}}
    @stack('css')
    ...
    @if (($InventoryManagerSettings) && ($InventoryManagerSettings->header_color!=''))
        <style nonce="{{ csrf_token() }}">
            ...
            .skin-{{ $InventoryManagerSettings->skin!='' ? $InventoryManagerSettings->skin : 'blue' }} .sidebar-menu > li:hover > a, .skin-{{ $InventoryManagerSettings->skin!='' ? $InventoryManagerSettings->skin : 'blue' }} .sidebar-menu > li.active > a {
    ...
    <script nonce="{{ csrf_token() }}">
        window.InventoryManager = {
            settings: {
                "per_page": {{ $InventoryManagerSettings->per_page }}
            }
        };
    </script>
...
@if (($InventoryManagerSettings) && ($InventoryManagerSettings->allow_user_skin==1) && Auth::check() && Auth::user()->present()->skin != '')
    <body class="sidebar-mini skin-{{ $InventoryManagerSettings->skin!='' ? Auth::user()->present()->skin : 'blue' }} {{ (session('menu_state')!='open') ? 'sidebar-mini sidebar-collapse' : ''  }}">
    @else
        <body class="sidebar-mini skin-{{ $InventoryManagerSettings->skin!='' ? $InventoryManagerSettings->skin : 'blue' }} {{ (session('menu_state')!='open') ? 'sidebar-mini sidebar-collapse' : ''  }}">
        @endif
                ...
                <nav class="navbar navbar-static-top" role="navigation">
                                ...
                                <a class="logo navbar-brand no-hover" href="{{ config('app.url') }}">
                                    @if ($InventoryManagerSettings->logo!='')
                                        <img class="navbar-brand-img"
                                             src="{{ Storage::disk('public')->url($InventoryManagerSettings->logo) }}"
                                             alt="{{ $InventoryManagerSettings->site_name }} logo">
                                    @endif
                                    {{ $InventoryManagerSettings->site_name }}
                                </a>
                                ...
                                <a class="logo navbar-brand no-hover" href="{{ config('app.url') }}">
                                    @if ($InventoryManagerSettings->logo!='')
                                        <img class="navbar-brand-img"
                                             ...
                                             alt="{{ $InventoryManagerSettings->site_name }} logo">
                                    @endif
                                    <span class="sr-only">{{ $InventoryManagerSettings->site_name }}</span>
                                </a>
                            @else
                                <a class="logo navbar-brand no-hover" href="{{ config('app.url') }}">
                                    {{ $InventoryManagerSettings->site_name }}
                    ...
                    <div class="navbar-custom-menu">
                        <ul class="nav navbar-nav">
                            ...
                            @can('admin')
                                @if ($InventoryManagerSettings->show_alerts_in_menu=='1')
