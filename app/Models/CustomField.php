...
class CustomField extends Model
{
     ...
     * do with previously existing values. - @InventoryManager
     *
    ...
    {
        return '_InventoryManager_'.preg_replace('/[^a-zA-Z0-9]/', '_', strtolower($name));
    }
    ...
    public function convertUnicodeDbSlug($original = null)
    {
        ...
        if (! function_exists('transliterator_transliterate')) {
            $long_slug = '_InventoryManager_'.str_slug(mb_convert_encoding(trim($name),"UTF-8"), '_');
        } else {
            $long_slug = '_InventoryManager_'.Utf8Slugger::slugify($name, '_');
        }
     ...
     * @see https://github.com/InventoryManager/Inventory Manager/issues/5896
     *
