 ...
 * the Inventory Manager Asset Management application.
 *
 * @version    v1.0
 */
class SettingsController extends Controller
{
     ...
     *
     * @author [A. Gianotto] [<InventoryManager@InventoryManager.net>]
     *
     * @since [v3.0]
     *
     * @return View
    ...
    public function getLdapSettings()
    {
        ...
        /**
         * This validator is only temporary (famous last words.) - @InventoryManager
         */
        $messages = [
    ...
    {
        if (! config('app.lock_passwords')) {
            Artisan::call('InventoryManager:backup', ['--filename' => 'manual-backup-'.date('Y-m-d-H-i-s')]);
    ...
    public function postRestore($filename = null)
    {
        ...
        if (! config('app.lock_passwords')) {
            ...
            if (Storage::exists($path.'/'.$filename)) {
                ...
                Artisan::call('InventoryManager:restore', 
                [
                    '--force' => true, 
                    '--no-progress' => true, 
                ...
                Artisan::call('InventoryManager:global-logout', ['--force' => true]);
        ...
        if (config('app.allow_purge')=='true') {
            ...
            if (!config('app.lock_passwords')) {
                ...
                if ($request->input('confirm_purge')=='DELETE') {
                    ...
                    Artisan::call('InventoryManager:purge', ['--force' => 'true', '--no-interaction' => true]);
                    $output = Artisan::output();
